...
| カーディナリティ | 要素の出現回数の制約 |
| 前像計算 | 出力集合から入力集合を逆算 |
| SMT | Satisfiability Modulo Theories（充足可能性判定理論） |
| Z3 | Microsoft製のSMTソルバー |
| 反例 | 妥当性が成立しない具体的な入力例 |
| 厳密検証 | SMTソルバーによる制約の完全な検証 |

---

## 更新履歴

- 2025-11-18: SMT厳密検証追加
  - Z3ソルバーによる制約の完全検証機能を追加
  - 具体的な反例値の自動生成
  - UI に「厳密検証 (SMT)」ボタンを追加
  - `backend/strict_validity_checker.py` を実装
  - `/api/validate-strict` エンドポイントを追加
  - ドキュメント更新（README.md, validity_checking.md）
